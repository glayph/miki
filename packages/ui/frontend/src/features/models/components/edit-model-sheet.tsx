  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await updateModel(model.index, {
        ...
        provider: provider,
        model: modelId,
        // null = explicit clear (backend must distinguish from omitted)
        api_base: submittedApiBase,
        api_key: form.apiKey.trim() || undefined,
        proxy: form.proxy.trim() || null,
        auth_method: authMethodLocked
          ? defaultAuthMethod || undefined
          : form.authMethod.trim() || null,
        connect_mode: form.connectMode.trim() || null,
        workspace: form.workspace.trim() || null,
        rpm: form.rpm ? Number(form.rpm) : null,
        max_tokens_field: form.maxTokensField.trim() || null,
        request_timeout: form.requestTimeout
          ? Number(form.requestTimeout)
          : null,
        thinking_level: form.thinkingLevel.trim() || null,
        tool_schema_transform: form.toolSchemaTransform.trim() || null,
        streaming,
