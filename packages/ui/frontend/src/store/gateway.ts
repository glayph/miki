...
export interface GatewayStoreState {
  ...
  runtimeApplyStatus?: "applied" | "pending_restart" | "failed"
...
function normalizeGatewayStoreState(
  ...
  if (
    ...
    next.runtimeApplyStatus === prev.runtimeApplyStatus &&
    next.runtimeApplyError === prev.runtimeApplyError &&
    next.pendingRestartFields.join("\0") ===
      prev.pendingRestartFields.join("\0")
  ) {
    return prev
  }
...
export function applyGatewayStatusToStore(
  ...
  updateGatewayStore((prev) => ({
    ...
    runtimeApplyStatus:
      prev.status === "stopping" && data.gateway_status === "running"
        ? prev.runtimeApplyStatus
        : (data.runtime_apply_status ?? prev.runtimeApplyStatus),
    runtimeApplyError:
      prev.status === "stopping" && data.gateway_status === "running"
        ? prev.runtimeApplyError
        : data.runtime_apply_error,
    pendingRestartFields:
      prev.status === "stopping" && data.gateway_status === "running"
        ? prev.pendingRestartFields
        : (data.pending_restart_fields ?? prev.pendingRestartFields),
  }))
}
