...
function SidebarRail({ className, ...props }: React.ComponentProps<"button">) {
  const { toggleSidebar } = useSidebar()
  const { t } = useTranslation()
  const label = t("navigation.toggle_sidebar")
  ...
  return (
    <button
      ...
      title={label}
      className={cn(
        "focus-visible:ring-sidebar-ring hover:after:bg-sidebar-border focus-visible:after:bg-sidebar-ring absolute inset-y-0 z-20 hidden w-4 transition-[background-color,transform] ease-linear group-data-[side=left]:-right-4 group-data-[side=right]:left-0 after:absolute after:inset-y-0 after:start-1/2 after:w-[2px] focus-visible:ring-2 focus-visible:outline-none sm:flex ltr:-translate-x-1/2 rtl:-translate-x-1/2",
...
function SidebarContent({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "flex min-h-0 flex-1 [scrollbar-width:none] flex-col gap-2 overflow-auto [-ms-overflow-style:none] group-data-[collapsible=icon]:overflow-hidden [&::-webkit-scrollbar]:hidden",
        className,
      )}
      {...props}
    />
