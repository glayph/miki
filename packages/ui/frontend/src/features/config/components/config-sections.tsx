...
export function AgentDefaultsSection({
  ...
  const modeLabel = (mode: TurnProfileMode) => {
    if (mode === "custom") return t("pages.config.turn_profile_mode_custom")
    if (mode === "off") return t("pages.config.turn_profile_mode_off")
    return t("pages.config.turn_profile_mode_default")
  }
  ...
  const renderContextOption = ({
    ...
    <div className="border-border/60 bg-muted/20 rounded-md border p-2.5">
      <div className="mb-1.5 flex items-center justify-between gap-2">
        <p className="text-[12px] leading-none font-semibold">{label}</p>
      </div>
      {select}
      <p className="text-muted-foreground mt-1.5 text-[11px] leading-4">
        {description}
      </p>
      {allowList && <div className="mt-2">{allowList}</div>}
    </div>
  )
  ...
  return (
    <ConfigSectionCard title={t("pages.config.sections.agent")}>
      ...
      <SwitchCardField
        label={t("pages.config.bypass_restrictions")}
        hint={t("pages.config.bypass_restrictions_hint")}
        layout="setting-row"
        checked={form.bypassRestrictions}
        onCheckedChange={(checked) =>
          onFieldChange("bypassRestrictions", checked)
        }
      />
      ...
      <Field
        ...
        hint={t("pages.config.turn_profile_hint_compact", {
          defaultValue: "Choose what context each request carries.",
        })}
        layout="setting-row"
      ...
      >
        <div className="space-y-2">
          <SwitchCardField
            ...
            hint={t("pages.config.turn_profile_enabled_hint_compact", {
              defaultValue: "Apply this policy to every new turn.",
            })}
          ...
          <Collapsible
            ...
            onOpenChange={setContextOptionsOpen}
          >
            <CollapsibleTrigger asChild>
              <Button
                ...
                variant="outline"
                size="sm"
                className="h-auto min-h-8 w-full justify-between gap-2 px-2.5 py-1.5 text-left"
              >
                <span className="min-w-0">
                  <span className="block text-xs font-semibold">
                    {t("pages.config.turn_profile_options", {
                      defaultValue: "Context options",
                    })}
                  </span>
                  <span className="text-muted-foreground mt-0.5 block truncate text-[11px] leading-4">
                    {[
                      modeLabel(form.turnProfile.historyMode),
                      modeLabel(form.turnProfile.systemPromptMode),
                      modeLabel(form.turnProfile.skillsMode),
                      modeLabel(form.turnProfile.toolsMode),
                    ].join(" / ")}
                  </span>
                </span>
                <IconChevronDown
            ...
            <CollapsibleContent>
              <div className="mt-2 grid gap-2 sm:grid-cols-2">
                {renderContextOption({
                  label: t("pages.config.turn_profile_history"),
                  description: t(
                    "pages.config.turn_profile_history_hint_compact",
                    {
                      defaultValue: "Earlier messages for this turn.",
                    },
                  ),
                  select: renderModeSelect({
                ...
                {renderContextOption({
                  label: t("pages.config.turn_profile_system_prompt"),
                  description: t(
                    "pages.config.turn_profile_system_prompt_hint_compact",
                    {
                      defaultValue:
                        "Identity, workspace, memory, and runtime context.",
                    },
                  ),
                  select: renderModeSelect({
                ...
                {renderContextOption({
                  label: t("pages.config.turn_profile_skills"),
                  description: t(
                    "pages.config.turn_profile_skills_hint_compact",
                    {
                      defaultValue: "Skill prompts available to the turn.",
                    },
                  ),
                  select: renderModeSelect({
                  ...
                  allowList: form.turnProfile.skillsMode === "custom" && (
                    <Textarea
                ...
                {renderContextOption({
                  label: t("pages.config.turn_profile_tools"),
                  description: t(
                    "pages.config.turn_profile_tools_hint_compact",
                    {
                      defaultValue: "Tool access available to the turn.",
                    },
                  ),
                  select: renderModeSelect({
                  ...
                  allowList: form.turnProfile.toolsMode === "custom" && (
                    <Textarea
