...
func TestFindSkillsAndInstallSkillStayEnabledWhenSpecificToolsAreEnabled(t *testing.T) {
	...
	if gotTools["install_skill"].Status != "enabled" {
		t.Fatalf("install_skill status = %q, want enabled", gotTools["install_skill"].Status)
	}
}
...
func TestHandleListTools(t *testing.T) {
	...
	if gotTools["spawn"].Status != "enabled" {
		t.Fatalf("spawn = %#v, want enabled", gotTools["spawn"])
	}
	if gotTools["find_skills"].Status != "enabled" {
		t.Fatalf("find_skills status = %q, want enabled", gotTools["find_skills"].Status)
	}
...
func TestHandleUpdateToolState(t *testing.T) {
	...
	if !updated.Tools.Spawn.Enabled {
		t.Fatalf("spawn should be enabled: %#v", updated.Tools)
	}
