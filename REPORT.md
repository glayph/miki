...
The first Windows workflow repair built `@miki/config` but core then exposed the next missing workspace dependency, `@miki/installer`. The Windows job was expanded to build config, installer, skills, memory, and core in the same dependency order as the Linux build. Run [#32749531405](https://github.com/glayph/miki/actions/runs/32749531405) passed both `linux-build-test` and `windows-platform-check`, including real LFS checkout, PowerShell parsing, Linux full validation, and Windows workspace compilation. This keeps Windows validation deterministic without downloading the large local model.
