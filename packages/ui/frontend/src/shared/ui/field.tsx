...
function FieldControlProvider({
  controlId,
  describedBy,
  children,
}: FieldControlContextValue & { children: ReactNode }) {
  const parentContext = useContext(FieldControlContext)
  const value = useMemo(
    () => ({
      controlId: controlId ?? parentContext?.controlId,
      describedBy: mergeIds(parentContext?.describedBy, describedBy),
    }),
    [
      controlId,
      describedBy,
      parentContext?.controlId,
      parentContext?.describedBy,
    ],
  )
...
function useFieldControl({
  ...
  return {
    ...
    "aria-describedby": mergeIds(describedBy, context?.describedBy),
  }
}
...
function Field({
  ...
  return (
    <div
      ...
      {...props}
    >
      <FieldControlProvider controlId={controlId}>
        {children}
