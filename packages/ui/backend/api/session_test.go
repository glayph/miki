...
func TestHandleListSessions_JSONLStorage(t *testing.T) {
	...
	sessionKey := legacymikiSessionPrefix + "history-jsonl"
...
func TestHandleListSessions_TransientThoughtDoesNotInflateMessageCount(t *testing.T) {
	...
	dir := sessionsTestDir(t, configPath)
	sessionKey := legacymikiSessionPrefix + "history-jsonl-transient"
...
func TestHandleListSessions_TitleUsesFirstUserMessage(t *testing.T) {
	...
	sessionKey := legacymikiSessionPrefix + "summary-title"
	if err := store.AddFullMessage(nil, sessionKey, providers.Message{
		Role:    "user",
...
func TestHandleGetSession_JSONLStorage(t *testing.T) {
	...
	sessionKey := legacymikiSessionPrefix + "detail-jsonl"
...
func TestHandleGetSession_HidesHandledToolAttachmentsBackedByMediaRefs(t *testing.T) {
	...
	sessionKey := legacymikiSessionPrefix + "attachment-history"
	for _, msg := range []providers.Message{
...
func TestHandleGetSession_ExposesHandledToolAttachmentsWithDurableURL(t *testing.T) {
	...
	sessionKey := legacymikiSessionPrefix + "attachment-history-durable"
	for _, msg := range []providers.Message{
		{Role: "user", Content: "send me the report"},
		{
			Role:    "assistant",
...
func TestHandleSessions_JSONLScopeDiscovery(t *testing.T) {
	...
	deleteReq.Header.Set(destructiveConfirmationHeader, "delete-session")
...
func TestHandleGetSession_SkipsTransientThoughtMessages(t *testing.T) {
	...
	sessionKey := mikiSessionPrefix + "detail-transient-thought"
	for _, msg := range []providers.Message{
...
func TestHandleGetSession_ReconstructsThoughtFromAssistantReasoningContent(t *testing.T) {
	...
	sessionKey := mikiSessionPrefix + "detail-reasoning-content"
	for _, msg := range []providers.Message{
		{Role: "user", Content: "hello"},
...
func TestHandleGetSession_ReconstructsRefreshMatrixForThoughtAndToolSummary(t *testing.T) {
	...
	sessionKey := mikiSessionPrefix + "detail-refresh-matrix"
	for _, msg := range []providers.Message{
...
func TestHandleGetSession_ReconstructsVisibleMessageToolOutputWithoutDuplicateSummary(t *testing.T) {
	...
	sessionKey := mikiSessionPrefix + "detail-message-tool"
	for _, msg := range []providers.Message{
		...
		{Role: "tool", Content: "Message sent to miki:miki:detail-message-tool", ToolCallID: "call_1"},
...
func TestHandleGetSession_PreservesFinalAssistantReplyAfterMessageToolOutput(t *testing.T) {
	...
	sessionKey := mikiSessionPrefix + "detail-message-tool-final-reply"
	for _, msg := range []providers.Message{
		{Role: "user", Content: "test"},
		{
			Role: "assistant",
		...
		{Role: "tool", Content: "Message sent to miki:miki:detail-message-tool-final-reply", ToolCallID: "call_1"},
...
func TestHandleListSessions_MessageCountUsesVisibleTranscript(t *testing.T) {
	...
	sessionKey := mikiSessionPrefix + "list-visible-count"
	for _, msg := range []providers.Message{
		{Role: "user", Content: "test"},
		{
			Role: "assistant",
		...
		{Role: "tool", Content: "Message sent to miki:miki:list-visible-count", ToolCallID: "call_1"},
		{Role: "assistant", Content: handledToolResponseSummaryText},
	} {
		if err := store.AddFullMessage(nil, sessionKey, msg); err != nil {
			t.Fatalf("AddFullMessage() error = %v", err)
...
func TestHandleListSessions_DeduplicatesAssistantToolCallContentFromVisibleTranscript(t *testing.T) {
	...
	sessionKey := mikiSessionPrefix + "list-deduped-tool-content"
	for _, msg := range []providers.Message{
...
func TestHandleGetSession_DoesNotDuplicateAssistantToolCallContent(t *testing.T) {
	...
	sessionKey := mikiSessionPrefix + "detail-tool-summary-and-content"
	for _, msg := range []providers.Message{
		{Role: "user", Content: "check file"},
		{
			Role:    "assistant",
...
func TestHandleGetSession_PreservesDistinctAssistantToolCallContent(t *testing.T) {
	...
	sessionKey := mikiSessionPrefix + "detail-tool-summary-distinct-content"
	for _, msg := range []providers.Message{
		{Role: "user", Content: "check file"},
		{
			Role:    "assistant",
...
func TestHandleGetSession_PreservesMediaWhenAssistantToolCallContentDuplicatesSummary(t *testing.T) {
	...
	sessionKey := mikiSessionPrefix + "detail-tool-summary-duplicate-content-with-media"
	for _, msg := range []providers.Message{
...
func TestHandleGetSession_PreservesAttachmentsWhenAssistantToolCallContentDuplicatesSummary(t *testing.T) {
	...
	sessionKey := mikiSessionPrefix + "detail-tool-summary-duplicate-content-with-attachments"
	for _, msg := range []providers.Message{
...
func TestHandleGetSession_UsesConfiguredToolFeedbackMaxArgsLength(t *testing.T) {
	...
	sessionKey := mikiSessionPrefix + "detail-tool-summary-max-args"
...
func TestHandleGetSession_FallsBackToLegacyToolArgumentsWhenExplanationMissing(t *testing.T) {
	...
	argsJSON := `{"path":"README.md","start_line":1,"end_line":10,"extra":"abcdefghijklmnopqrstuvwxyz"}`
	sessionKey := mikiSessionPrefix + "detail-tool-summary-legacy-args"
...
func TestHandleGetSession_IncludesMediaOnlyMessages(t *testing.T) {
	...
	sessionKey := mikiSessionPrefix + "detail-media-only"
	if err := store.AddFullMessage(nil, sessionKey, providers.Message{
		Role:  "user",
		Media: []string{"data:image/png;base64,abc123"},
	}); err != nil {
...
func TestHandleSessions_SupportsJSONLMessagesUpToStoreCap(t *testing.T) {
	...
	sessionKey := mikiSessionPrefix + "detail-large-jsonl"
...
func TestHandleListSessions_UsesImagePreviewForMediaOnlyMessage(t *testing.T) {
	...
	sessionKey := mikiSessionPrefix + "preview-media-only"
	if err := store.AddFullMessage(nil, sessionKey, providers.Message{
		Role:  "user",
		Media: []string{"data:image/png;base64,abc123"},
	}); err != nil {
...
func TestHandleDeleteSession_JSONLStorage(t *testing.T) {
	...
	sessionKey := legacymikiSessionPrefix + "delete-jsonl"
	if err := store.AddFullMessage(nil, sessionKey, providers.Message{
		Role:    "user",
		Content: "delete me",
	}); err != nil {
	...
	req.Header.Set(destructiveConfirmationHeader, "delete-session")
	mux.ServeHTTP(rec, req)
...
func TestHandleDeleteSessionRequiresConfirmation(t *testing.T) {
	configPath, cleanup := setupOAuthTestEnv(t)
	defer cleanup()
	...
	sessionKey := legacymikiSessionPrefix + "confirm-delete-jsonl"
	if err := store.AddFullMessage(nil, sessionKey, providers.Message{
		Role:    "user",
		Content: "delete requires confirmation",
	}); err != nil {
		t.Fatalf("AddFullMessage() error = %v", err)
	}
	...
	rec := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodDelete, "/api/sessions/confirm-delete-jsonl", nil)
	mux.ServeHTTP(rec, req)
	...
	if _, err := os.Stat(base + ".jsonl"); err != nil {
		t.Fatalf("session file should remain after unconfirmed delete, stat err = %v", err)
	}
}
...
func TestHandleGetSession_LegacyJSONFallback(t *testing.T) {
	...
	sessionKey := legacymikiSessionPrefix + "legacy-json"
...
func TestHandleSessions_FiltersEmptyJSONLFiles(t *testing.T) {
	...
	dir := sessionsTestDir(t, configPath)
	base := filepath.Join(dir, sanitizeSessionKey(legacymikiSessionPrefix+"empty-jsonl"))
...
func TestHandleSessions_ListsLegacyJSONLWithoutMeta(t *testing.T) {
	...
	dir := sessionsTestDir(t, configPath)
	sessionKey := legacymikiSessionPrefix + "missing-meta"
	base := filepath.Join(dir, sanitizeSessionKey(sessionKey))
...
func TestHandleSessions_IgnoresMetaJSONInLegacyFallback(t *testing.T) {
	...
	dir := sessionsTestDir(t, configPath)
	metaOnly := filepath.Join(dir, "agent_main_miki_direct_miki_meta-only.meta.json")
	metaOnlyContent := []byte(`{"key":"agent:main:miki:direct:miki:meta-only","summary":"meta only"}`)
