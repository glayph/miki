...
export const OpenClawPage: React.FC = () => {
  const doc = getDocByPath('/docs/ecosystem/openclaw.md');
    ...
    >
      <div className="mb-8">
        <div className="flex items-center gap-2 mb-3">
          <span className="text-[11px] font-mono uppercase tracking-widest px-2.5 py-1 rounded bg-[#111113] border border-[#27272A] text-[#FF5A3C] font-bold">
            ECOSYSTEM / OPENCLAW ADAPTER
          </span>
          <span className="text-xs font-mono text-[#A1A1AA]">Universal JSON Skill Loader</span>
        </div>
        ...
        <p className="text-[#A1A1AA] text-base max-w-3xl leading-relaxed">
          The OpenClaw Skill Adapter maps existing OpenClaw JSON tool definitions into native Miki ISkillPlugin interfaces without code changes or schema rewrites.
        </p>
      </div>
      ...
      {doc ? (
        ...
        <div className="p-8 bg-[#111113] border border-[#27272A] rounded-xl text-xs font-mono text-[#A1A1AA]">
          Document /docs/ecosystem/openclaw.md not found.
        </div>
      )}
    </motion.div>
  );
