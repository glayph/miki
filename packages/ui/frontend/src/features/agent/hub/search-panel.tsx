      ...
      >
        <div className="group relative flex items-center justify-center">
          <Input
            ...
            aria-label={t("pages.agent.skills.marketplace_search_placeholder")}
            name="agent_hub_search"
            autoComplete="off"
            placeholder={t("pages.agent.skills.marketplace_search_placeholder")}
            className="border-border/60 bg-background/50 hover:bg-background focus-visible:ring-primary/20 h-12 w-full rounded-full pr-20 pl-5 text-sm shadow-sm backdrop-blur-sm transition-[background-color,border-color,box-shadow] focus-visible:ring-2 md:min-w-[520px]"
            disabled={!canSearchMarketplace}
          />
          <Button
            type="submit"
            className="absolute top-1/2 right-1.5 h-9 -translate-y-1/2 rounded-full px-4 font-medium shadow-sm transition-[background-color,color,box-shadow,transform]"
            disabled={
              !canSearchMarketplace ||
              isMarketSearchInitialLoading ||
              marketQuery.trim() === ""
