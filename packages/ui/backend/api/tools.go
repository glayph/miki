...
import (
	...
	mikitools "github.com/sipeed/miki/pkg/tools"
)
...
var toolCatalog = []toolCatalogEntry{
	...
	{
		Name:        "spawn",
		Description: "Launch a background worker for long-running or delegated work.",
		Category:    "automation",
		ConfigKey:   "spawn",
	},
	{
		Name:        "spawn_status",
		Description: "Query the status of background workers.",
		Category:    "automation",
...
func buildToolSupport(cfg *config.Config) []toolSupportItem {
	...
	for _, entry := range toolCatalog {
		status := "disabled"
		reasonCode := ""
		configEnabled := cfg.Tools.IsToolEnabled(entry.ConfigKey) || cfg.Tools.IsToolEnabled("skills")

		switch entry.Name {
		case "find_skills", "install_skill":
			if cfg.Tools.IsToolEnabled(entry.ConfigKey) || cfg.Tools.IsToolEnabled("skills") {
				status = "enabled"
			}
		...
		case "tool_search_tool_regex":
			configEnabled = cfg.Tools.MCP.Discovery.UseRegex
			status, reasonCode = resolveDiscoveryToolSupport(cfg, cfg.Tools.MCP.Discovery.UseRegex)
		case "tool_search_tool_bm25":
			configEnabled = cfg.Tools.MCP.Discovery.UseBM25
...
func buildWebSearchConfigResponse(cfg *config.Config) webSearchConfigResponse {
	opts := mikitools.WebSearchToolOptionsFromConfig(cfg)
	...
	providers := []webSearchProviderOption{
		...
		},
		{
			ID:         "sogou",
			Label:      "Sogou",
			Configured: mikitools.WebSearchProviderReady(opts, "sogou"),
			Current:    current == "sogou",
		},
		{
			ID:         "duckduckgo",
			Label:      "DuckDuckGo",
			Configured: mikitools.WebSearchProviderReady(opts, "duckduckgo"),
			Current:    current == "duckduckgo",
		},
		{
			ID:           "gemini",
			Label:        "Gemini (Google Search)",
			Configured:   mikitools.WebSearchProviderReady(opts, "gemini"),
		...
		},
		{
			ID:           "brave",
			Label:        "Brave Search",
			Configured:   mikitools.WebSearchProviderReady(opts, "brave"),
		...
		},
		{
			ID:           "tavily",
			Label:        "Tavily",
			Configured:   mikitools.WebSearchProviderReady(opts, "tavily"),
		...
		},
		{
			ID:           "perplexity",
			Label:        "Perplexity",
			Configured:   mikitools.WebSearchProviderReady(opts, "perplexity"),
		...
		},
		{
			ID:         "searxng",
			Label:      "SearXNG",
			Configured: mikitools.WebSearchProviderReady(opts, "searxng"),
			Current:    current == "searxng",
		},
		{
			ID:           "glm_search",
			Label:        "GLM Search",
			Configured:   mikitools.WebSearchProviderReady(opts, "glm_search"),
		...
		},
		{
			ID:           "baidu_search",
			Label:        "Baidu Search",
			Configured:   mikitools.WebSearchProviderReady(opts, "baidu_search"),
...
func resolveCurrentWebSearchProvider(cfg *config.Config) string {
	...
	selected, err := mikitools.ResolveWebSearchProviderName(mikitools.WebSearchToolOptionsFromConfig(cfg), "")
	if err != nil {
		return ""
	}
	return selected
