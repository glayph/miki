...
export function useSkillsPage() {
  ...
  const pluginReadinessQuery = useQuery({
    queryKey: ["skills", "plugin-marketplace", "readiness"],
    queryFn: getPluginMarketplaceReadiness,
    staleTime: 60 * 1000,
  })
  ...
  const importMutation = useMutation({
    ...
    onSuccess: (importedSkill) => {
      const message = t("pages.agent.skills.import_success")
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: (_, deletedName) => {
      const message = t("pages.agent.skills.delete_success")
      setStatusMessage({ kind: "success", text: message })
      toast.success(message)
  ...
  return {
    ...
    pluginReadiness: pluginReadinessQuery.data,
    skillsError: skillsQuery.error,
    skillDetailError: skillDetailQuery.error,
    pluginReadinessError: pluginReadinessQuery.error,
    isLoading: skillsQuery.isLoading,
    isPluginReadinessLoading: pluginReadinessQuery.isLoading,
