...
func (h *Handler) handleUpdateConfig(w http.ResponseWriter, r *http.Request) {
	...
	if err = normalizeChannelListShape(raw); err != nil {
		http.Error(w, fmt.Sprintf("Invalid channel_list: %v", err), http.StatusBadRequest)
		return
	}
...
func (h *Handler) handlePatchConfig(w http.ResponseWriter, r *http.Request) {
	...
	if err = normalizeChannelListShape(patch); err != nil {
		http.Error(w, fmt.Sprintf("Invalid channel_list: %v", err), http.StatusBadRequest)
		return
	}
	...
	mergeMap(base, patch)
	if err = normalizeChannelListShape(base); err != nil {
		http.Error(w, fmt.Sprintf("Invalid channel_list: %v", err), http.StatusBadRequest)
		return
	}
...
func normalizeChannelListShape(raw map[string]any) error {
	rawChannels, exists := raw["channel_list"]
	if !exists || rawChannels == nil {
		return nil
	}

	switch channels := rawChannels.(type) {
	case map[string]any:
		return nil
	case []any:
		normalized := make(map[string]any, len(channels))
		for index, rawChannel := range channels {
			channelMap, ok := rawChannel.(map[string]any)
			if !ok {
				return fmt.Errorf("channel_list[%d] must be an object", index)
			}
			name, _ := channelMap["name"].(string)
			if strings.TrimSpace(name) == "" {
				name, _ = channelMap["type"].(string)
			}
			name = strings.TrimSpace(name)
			if name == "" {
				return fmt.Errorf("channel_list[%d] must include name or type", index)
			}
			cloned := make(map[string]any, len(channelMap))
		...
		raw["channel_list"] = normalized
		return nil
	default:
		return fmt.Errorf("must be an object or array, got %T", rawChannels)
	}
}
