...
function asOptionalPositiveNumberString(
  value: unknown,
  fallback: string,
): string {
  if (typeof value === "number" && Number.isFinite(value)) {
...
export function buildFormFromConfig(config: unknown): CoreConfigForm {
  ...
  const security = asRecord(agent.security)
  ...
  return {
    ...
    bypassRestrictions:
      security.bypass_restrictions === undefined
        ? EMPTY_FORM.bypassRestrictions
        : asBool(security.bypass_restrictions),
    splitOnMarker:
...
export function parseOptionalPositiveIntField(
  rawValue: string,
  label: string,
): number | undefined {
  ...
  return parseIntField(trimmed, label, { min: 1 })
}
