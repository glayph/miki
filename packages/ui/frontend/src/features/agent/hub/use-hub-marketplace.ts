...
export function useHubMarketplace() {
  ...
  const skillSearchHealthQuery = useQuery({
    queryKey: ["skills-api-health"],
    queryFn: async () => {
      try {
        await searchSkills("test", 1, 0)
        return { available: true, error: null as string | null }
      } catch (err) {
        return {
          available: false,
          error: err instanceof Error ? err.message : "Skill API unavailable",
        }
      }
    },
    staleTime: 60_000,
  ...
  const skillInstallHealthQuery = useQuery({
    queryKey: ["skills-install-api-health"],
    queryFn: async () => {
      try {
        await getSkills()
        return { available: true, error: null as string | null }
  ...
  const unavailableToolMessages = buildUnavailableToolMessages({
    searchTool: canSearchMarketplace
      ? undefined
      : { status: "disabled" as const, reason_code: undefined },
    installTool: canInstallFromMarketplace
      ? undefined
      : { status: "disabled" as const, reason_code: undefined },
    t,
  })
  ...
  const handleInstall = (result: SkillRegistrySearchResult) => {
    installMutation.mutate({
      ...
      id: result.id,
      url: result.url,
      registry: result.registry_name || "unknown",
  ...
  const isInstallPending = (result: SkillRegistrySearchResult) =>
    installPendingKey === `${result.registry_name || "unknown"}:${result.slug}`
