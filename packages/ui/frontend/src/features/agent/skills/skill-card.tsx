...
export function SkillCard({ skill, onView, onDelete }: SkillCardProps) {
  ...
  return (
    <Card
      className="group border-border/40 bg-card/40 hover:bg-card hover:border-border/80 relative overflow-hidden transition-[background-color,border-color,box-shadow] hover:shadow-md"
      size="sm"
    >
        ...
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="flex items-center gap-1 opacity-80 transition-opacity group-hover:opacity-100">
            <Button
              ...
              onClick={onView}
              aria-label={t("pages.agent.skills.view")}
            ...
            {skill.source === "workspace" ? (
              <Button
                ...
                onClick={onDelete}
                aria-label={t("pages.agent.skills.delete")}
      ...
      <CardContent>
        {skill.registry_url ? (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="text-primary/80 hover:text-primary inline-flex max-w-full items-center text-xs break-all transition-colors hover:underline hover:underline-offset-4"
            title={skill.registry_url}
          >
            {skill.registry_url}
          </a>
        ) : null}
