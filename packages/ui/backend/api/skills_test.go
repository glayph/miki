...
func TestHandleDeleteSkill(t *testing.T) {
	...
	req.Header.Set(destructiveConfirmationHeader, "delete-skill")
	mux.ServeHTTP(rec, req)
...
func TestHandleDeleteSkillRequiresConfirmation(t *testing.T) {
	configPath, cleanup := setupOAuthTestEnv(t)
	defer cleanup()
	...
	skillDir := filepath.Join(workspace, "skills", "confirm-delete")
	if err := os.MkdirAll(skillDir, 0o755); err != nil {
		t.Fatalf("MkdirAll() error = %v", err)
	}
	if err := os.WriteFile(
	...
	rec := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodDelete, "/api/skills/confirm-delete", nil)
	mux.ServeHTTP(rec, req)
	...
	if _, err := os.Stat(skillDir); err != nil {
		t.Fatalf("skill directory should remain after unconfirmed delete, stat err=%v", err)
	}
}
